...
#endif
{
   int argc_pos = 0;
   // /*
   // do_config_test=0 notest 
   // do_config_test=1 test
   // */
...
// #endif /* defined(_WIN32) */
      ...
      //   if (strcmp(argv[argc_pos], "--no-daemon") == 0)
      // {
      //    set_debug_level(LOG_LEVEL_FATAL | LOG_LEVEL_ERROR | LOG_LEVEL_INFO);
      //    daemon_mode = 0;
      // }

        if (strcmp(argv[argc_pos], "--pidfile") == 0)
      {
      ...
      else if (strcmp(argv[argc_pos], "--pre-chroot-nslookup") == 0)
      {
         ...
         debugs(pre_chroot_nslookup_to_load_resolver);
      }
...
//    }
   ...
   initialize the struct of the cgi_error_memory_responsecgi_error_memory_response 
   */
   cgi_init_error_messages();
